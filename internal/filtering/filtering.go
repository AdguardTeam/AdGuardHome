...
func filterDNSResultByListIDs(
	...
	// Recompute NetworkRule (the "best" non-dnsrewrite rule) if the current
	// one was filtered out or was nil.
	needRecompute := dnsres.NetworkRule == nil ||
		!isAllowed(dnsres.NetworkRule.GetFilterListID())
	if needRecompute {
		dnsres.NetworkRule = nil

		if len(dnsres.NetworkRules) > 0 {
			// Find replacement: whitelist (exception) rules take
			// priority.
