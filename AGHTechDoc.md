...
- `default`: Behaves the same as `null_ip` — responds with a zero IP address (0.0.0.0 for A; :: for AAAA) when blocked by a rule.
- `refused`: Responds with a **REFUSED** DNS status code.
- `nxdomain`: Respond with an **NXDOMAIN** DNS status code.
- `null_ip`: Respond with zero IP address (0.0.0.0 for A; :: for AAAA).
- `custom_ip`: Responds with a manually configured IP address specified by `blocking_ipv4` and/or `blocking_ipv6`.
...
. aggregate data for "top\_" output arrays; sort in descending order
