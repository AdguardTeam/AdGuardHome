# AGENTS.md — AdGuard Home client_v3 (SolidJS Frontend)
...
`client_v3` is the next-generation web UI for **AdGuard Home**, written in
...
All commands are run from the `client_v3/` directory.
...
`client_v3` is a browser-resident SPA — it runs entirely client-side and has
